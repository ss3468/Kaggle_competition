...
class Lfrontage:
    ...
    def fit(self,df):
        data=df[['LotArea','Neighborhood','LotShape','LotConfig','LotFrontage']].copy()
        data['LotArea_transformed']=np.log1p(data['LotArea'])
        n = len(data['LotArea_transformed'])
        self.lotfrontage_std=data['LotFrontage'].std()
        ...
        data['LotArea_Bins'] = pd.cut(data['LotArea_transformed'], bins=self.bin_edges,labels=[f"Bin {i+1}" for i in range(len(self.bin_edges) - 1)])
        self.df_group_1=data.groupby(['Neighborhood','LotShape','LotConfig','LotArea_Bins'])['LotFrontage'].median().reset_index()
        self.df_group_2=data.groupby(['Neighborhood','LotShape','LotConfig'])['LotFrontage'].median().reset_index()
    ...
    def transform(self,df):
        if(self.df_group_1.empty)|(self.df_group_2.empty)|(self.df_group_3.empty):
            ...
            df['LotArea_transformed']=np.log1p(df['LotArea'])
            ...
            # df['LotArea_Bins']=np.where(df['LotArea_Bins'].isnull()&(df['LotArea_transformed']<=self.bin_edges[0]),"Bin 0",df['LotArea_Bins'])
            # print([f"Bin {i}" for i in range(len(self.bin_edges) - 1)])
            noise_mean = 0       # Centered at 0
            noise_std = 0.1*self.lotfrontage_std      # Small standard deviation
            noise = np.random.normal(noise_mean, noise_std, size=len(df))
            df=pd.merge(df, self.df_group_1, on=['Neighborhood','LotShape','LotConfig','LotArea_Bins'], how='left', suffixes=('', '_1'))
            df['LotFrontage']=np.where(df['LotFrontage'].isnull(),round(df['LotFrontage_1']+noise,2),df['LotFrontage'])
            df=pd.merge(df, self.df_group_2, on=['Neighborhood','LotShape','LotConfig'], how='left', suffixes=('', '_2'))
            df['LotFrontage']=np.where(df['LotFrontage'].isnull(),round(df['LotFrontage_2']+noise,2),df['LotFrontage'])
            ...
            df.drop(columns=['LotArea_transformed','LotArea_Bins','LotFrontage_1','LotFrontage_2','LotFrontage_3','LotFrontage_4','LotFrontage_broad'], inplace=True)
        return df
