...
def clean_slang(text):
    ...
    text = re.sub(r"\bplz\b",'please',text,flags=re.I)
    # date\time adjustments
    ...
    text=re.sub(r"\b3p - 3\\:30a\b","3 PM - 3:30 AM",text)
    ...
    text=re.sub(r"\bm8\b","mate",text,flags=re.I)
    text=re.sub(r"\betc\b","and so on",text,flags=re.I)
    text=re.sub(r"\bq\&a\b","question and answer",text,flags=re.I)
    text=re.sub(r"\bfaq\b","frequently asked questions",text,flags=re.I)
    # acronymns
...
def expand_contractions(text):
    ...
    # Use a regular expression to find words without periods with or without apostrophes
    pattern = re.compile(r"\b(?:\w+(?:'\w+)?)(?!\.)\b")
    expanded_text = pattern.sub(custom_replace, text)
    expanded_text=re.sub(r"\buve\b","you have",expanded_text)
    expanded_text=re.sub(r"\bgov\'t\b","government",expanded_text,flags=re.I)

    return expanded_text
def remove_punctuation_except_dates_times_roads(sentence):
    # Define regex patterns for dates, times, roads, and highways
    date_pattern = r'\b(?:\d{1,2}[-/]\d{1,2}[-/]\d{2,4}|\d{1,2}[-/]\d{1,2}[-/]\d{2,4}|\d{1,2}[/]\d{1,2}|\d{1,2}[/]\d{2,4})\b'
    time_pattern = r'\b(?:\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM|am|pm)?|noon|midnight)\b'
    road_pattern = r'\b(?:road|rd|highway|hwy|street|st|avenue|ave|lane|ln|boulevard|blvd|drive|dr)\b'
    highway_pattern = r'\b(?:I-[1-9]\d{0,2}|US-[1-9]\d{0,2}|[NSWE]\d{1,4}(?:\s*(?:North|South|East|West))?)\b'
    
    # Combine all patterns into a single regex pattern
    combined_pattern = r'({})|({})|({})|({})|(\b(?:\w\.?)+\b)'.format(date_pattern, time_pattern, road_pattern, highway_pattern)
    
    # Find all matches and join them back into a sentence
    matches = re.findall(combined_pattern, sentence)
    cleaned_sentence = ' '.join(''.join(match) for match in matches if any(match))
...
def clean_text(text):
    ...
    text=remove_punctuation_except_dates_times_roads(text)
