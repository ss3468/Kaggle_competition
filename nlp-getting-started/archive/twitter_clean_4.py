...
def clean_slang(text):
    ...
    text=re.sub(r"\bbb\b","bye bye",text)
    ...
    text = re.sub(r"(\bb/c\b)|(bcs)|(\bbc\b)|(\bcuz\b)|(\bbcuz\b)", "because", text)#text = re.sub(r"\bb/c\b", "because", text)
    ...
    text=re.sub(r"(\bfirefighte\b)|(\bfirefightr\b)|(\bfirefigthers\b)","firefighter",text,flags=re.I)
...
def remove_irr_char_func(text):
    text= re.sub('\d{1,2}\:\d{2}(\s)?(AM|am|PM|pm)', '', text)
